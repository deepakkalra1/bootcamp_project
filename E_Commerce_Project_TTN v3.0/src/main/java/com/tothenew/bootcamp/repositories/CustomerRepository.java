...
public interface CustomerRepository extends CrudRepository<Customer,Integer> {
    ...
    @Query(value = "select user.id,user.created_date,user.created_by,user.last_modified_date,user.last_modified_by,first_name,middle_name,last_name,email,user.version,contact,is_active,is_deleted,password,image from user,user_role where user.id=user_role.user_id AND user_role.role_id=3",nativeQuery = true)

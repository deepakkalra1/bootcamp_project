...
import com.tothenew.bootcamp.pojo.CategoryPojo;
...
public class CategoryService {
    ...
    public CommonResponseVO<LinkedHashMap> viewSingleCategoryByAdmin(int id){
        CommonResponseVO<LinkedHashMap> commonResponseVO = new CommonResponseVO<>();
            ...
            LinkedHashMap<String, LinkedHashMap> hashMap = new LinkedHashMap<>();

        Category category = categoryRespository.findById(id).get();
            CategoryPojo categoryPojo = new CategoryPojo();
            categoryPojo.setCategoryId(category.getId());
            categoryPojo.setCategoryName(category.getName());
            try {

    categoryPojo.setParentName(category.getParentCategory().getName());
                categoryPojo.setParentId(category.getParentCategory().getId());
            }catch (NullPointerException e){

            }
            LinkedHashMap<String, CategoryPojo> mainCategoryPojohashmap = new LinkedHashMap<>();
            mainCategoryPojohashmap.put(String.valueOf(id),categoryPojo);
            hashMap.put("category",mainCategoryPojohashmap);
...
//            category.setProductSet(productList);
    ...
    //-------------------------------------------------------------------------------------------------------->
    public CommonResponseVO updateCategoryByAdmin(HashMap<String, String> categoryUpdateData){
        if (categoryUpdateData.size()!=2){
            throw new GiveMessageException(Arrays.asList(StatusCode.NOT_VALID_FORMAT.toString())
                    ,Arrays.asList("categoryId or newCategoryName or both key were not provided"));
        }
        Integer categoryIdObj = Integer.parseInt( categoryUpdateData.get("categoryId"));
        String newCategoryName = categoryUpdateData.get("newCategoryName");
        if (categoryIdObj==null || newCategoryName==null){
            throw new GiveMessageException(Arrays.asList(StatusCode.NOT_VALID_FORMAT.toString())
            ,Arrays.asList("Key name is not valid"));
        }
        ...
        findBrandNameDownInTree(categoryId,brandNames,productPrices);
            ...
            if (productPrices.size()==0){
                throw new GiveMessageException(Arrays.asList(StatusCode.EMPTY.toString()),
                        Arrays.asList("Product may be available but No product variation available for that product to fetch min and max price"));
            }
            else {

...
public interface SellerRepository extends CrudRepository<Seller, Integer> {
    ...
    @Query(value = "select user.id,user.creation_date,user.last_modified_date,first_name,middle_name,last_name,email,image,user.version,contact,is_active,is_deleted,password  from user,user_role where user.id=user_role.user_id AND user_role.role_id=2",nativeQuery = true)
